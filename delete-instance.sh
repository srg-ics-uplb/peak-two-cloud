...
Q=`cat <<EOF
SELECT id FROM nova.instances WHERE instances.display_name = '$VMNAME' order by id desc limit 1;
SELECT id,uuid FROM nova.instances WHERE instances.display_name = '$VMNAME' order by id desc limit 1;
EOF`
